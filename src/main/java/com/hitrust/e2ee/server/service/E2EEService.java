...
public class E2EEService {
	private static Logger LOG = Logger.getLogger(E2EEService.class);
...
//			byte[] result = cipher.doFinal( hexStr2Bin(plaintext));
	...
	public  Boolean verifyRawData(String data,String encDBData){
		try{
	        ...
	        cipher.init(Cipher.ENCRYPT_MODE, key);
			byte[] result =cipher.doFinal(secretKey.getEncoded());
			if( e2ee.verifyPassWord(Utility.bin2HexStr(byteCipherText), Utility.bin2HexStr(result),encDBData).intValue() == 0){
				return Boolean.TRUE;
			}else{
