...
public class E2EEService {
    ...
    public String encrypt(String encData, String encKeyWithRSA) {
        return this.e2ee.setPassWord(encData, encKeyWithRSA);
    }
    ...
    public String encryptRawData(String data) {
        try {
            ...
            BigInteger modulus = new BigInteger(this.Modulus, 16);
            BigInteger exponent = new BigInteger(this.Exponent, 16);
            RSAPublicKeySpec publicKeySpec = new RSAPublicKeySpec(modulus, exponent);
            KeyFactory factory = KeyFactory.getInstance("RSA");
            RSAPublicKey key = (RSAPublicKey) factory.generatePublic(publicKeySpec);
            Cipher cipher2 = Cipher.getInstance("RSA/ECB/PKCS1Padding");
            cipher2.init(1, key);
            byte[] result = cipher2.doFinal(secretKey.getEncoded());
            return this.e2ee.setPassWord(Utility.bin2HexStr(byteCipherText), Utility.bin2HexStr(result));
        } catch (Exception e) {
    ...
    public Boolean verifyRawData(String data, String encDBData) {
        try {
            ...
            cipher2.init(1, key);
            byte[] result = cipher2.doFinal(secretKey.getEncoded());
            if (this.e2ee.verifyPassWord(Utility.bin2HexStr(byteCipherText), Utility.bin2HexStr(result), encDBData).intValue() == 0) {
                return Boolean.TRUE;
            }
    ...
    public Boolean verify(String encData, String encKeyWithRSA, String encDBData) {
        if (this.e2ee.verifyPassWord(encData, encKeyWithRSA, encDBData).intValue() == 0) {
            return Boolean.TRUE;
        }
        return Boolean.FALSE;
    }
